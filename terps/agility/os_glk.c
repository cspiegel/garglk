 ...
 */
void
agt_statline (const char *cp_string)
{
  assert (cp_string);
  ...
  if (height > 1)
    {
       ...
       */
      glk_window_move_cursor (gagt_status_window, 0, 1);
      glk_put_string ("  Exits: ");
      for (exit = 0; exit < sizeof (exitname) / sizeof (exitname[0]); exit++)
        {
          if (compass_rose & (1 << exit))
            {
              glk_put_string ((char *) exitname[exit]);
              glk_put_char (' ');
            }
        }
                    ...
                    unsigned char packed_attributes)
{
  ...
  if (bytes > buffer->allocation)
    {
      buffer->data = gagt_realloc (buffer->data, bytes);
 ...
 */
static int
gagt_get_paragraph_line_count (const gagt_paragraphref_t paragraph)
{
  assert (paragraph && paragraph->magic == GAGT_PARAGRAPH_MAGIC);
 ...
 */
static int
gagt_compare_special_line (const char *compare, const gagt_lineref_t line)
{
  /*
...
static int
gagt_compare_special_paragraph (const gagt_specialref_t special,
 ...
 */
static glui32
gagt_display_special (const gagt_specialref_t special, glui32 current_style)
{
  glui32 set_style;
 ...
 */
static void
gagt_display_register_help_request (void)
{
  gagt_help_requested = TRUE;
}

static void
gagt_display_silence_help_hints (void)
{
 ...
 */
static glui32
gagt_display_line (const gagt_lineref_t line, glui32 current_style,
 ...
 */
static glui32
gagt_display_hinted_line (const gagt_lineref_t line, glui32 current_style,
...
static void
gagt_styled_char (glui32 style, char c)
{
  char buffer[2];
...
static void
gagt_header_string (const char *message)
{
  ...
  if (gagt_strcasecmp (argument, "on") == 0)
    {
      ...
      glk_window_set_echo_stream (gagt_main_window, gagt_transcript_stream);
  ...
  if (gagt_strcasecmp (argument, "on") == 0)
    {
      ...
      gagt_inputlog_stream = glk_stream_open_file (fileref,
                                                   filemode_WriteAppend, 0);
      glk_fileref_destroy (fileref);
      if (!gagt_inputlog_stream)
        {
  ...
  if (gagt_strcasecmp (argument, "on") == 0)
    {
      ...
      gagt_readlog_stream = glk_stream_open_file (fileref, filemode_Read, 0);
      glk_fileref_destroy (fileref);
      if (!gagt_readlog_stream)
        {
          gagt_standout_string ("Glk read log failed.\n");
 ...
 */
static void
gagt_command_license (const char *argument)
{
  assert (argument);

  gagt_normal_string ("This program is free software; you can redistribute it"
                      " and/or modify it under the terms of version 2 of the"
                      " GNU General Public License as published by the Free"
                      " Software Foundation.\n\n");

  gagt_normal_string ("This program is distributed in the hope that it will be"
                      " useful, but ");
  gagt_standout_string ("WITHOUT ANY WARRANTY");
  gagt_normal_string ("; without even the implied warranty of ");
  gagt_standout_string ("MERCHANTABILITY");
  gagt_normal_string (" or ");
  gagt_standout_string ("FITNESS FOR A PARTICULAR PURPOSE");
  gagt_normal_string (".  See the GNU General Public License for more"
                      " details.\n\n");

  gagt_normal_string ("You should have received a copy of the GNU General"
                      " Public License along with this program; if not, write"
                      " to the Free Software Foundation, Inc., 59 Temple"
                      " Place, Suite 330, Boston, MA  02111-1307 USA\n\n");

  gagt_normal_string ("Please report any bugs, omissions, or misfeatures to ");
  gagt_standout_string ("simon_baldwin@yahoo.com");
  gagt_normal_string (".\n");
}
...
static gagt_command_t GAGT_COMMAND_TABLE[] = {
  ...
  {"license",        gagt_command_license,        FALSE},
 ...
 */
static void
gagt_command_summary (const char *argument)
{
  ...
  for (entry = GAGT_COMMAND_TABLE; entry->command; entry++)
    {
      if (entry->handler == gagt_command_summary
            || entry->handler == gagt_command_license
 ...
 */
static void
gagt_command_help (const char *command)
{
  ...
  if (strlen (command) == 0)
    {
      ...
      gagt_normal_string (" followed by a Glk command name for help on that"
                          " command.\n");
      return;
    }
  ...
  for (entry = GAGT_COMMAND_TABLE; entry->command; entry++)
    {
      if (gagt_strncasecmp (command, entry->command, strlen (command)) == 0)
        {
          if (matched)
            {
  ...
  if (matched->handler == gagt_command_summary)
    ...
    {
      gagt_normal_string ("Records the commands you type into a game.\n\nUse ");
      gagt_standout_string ("glk inputlog on");
      gagt_normal_string (", to begin recording your commands, and ");
      ...
      gagt_normal_string (" command.\n");
    }

  else if (matched->handler == gagt_command_readlog)
    {
      gagt_normal_string ("Plays back commands recorded with ");
      gagt_standout_string ("glk inputlog on");
      gagt_normal_string (".\n\nUse ");
      gagt_standout_string ("glk readlog on");
      gagt_normal_string (".  Command play back stops at the end of the"
    ...
    {
      gagt_normal_string ("Turn off Glk commands.\n\nUse ");
      gagt_standout_string ("glk commands off");
      gagt_normal_string (" to disable all Glk commands, including this one."
                          "  Once turned off, there is no way to turn Glk"
                          " commands back on while inside the game.\n");
    }

  else if (matched->handler == gagt_command_license)
    {
      gagt_normal_string ("Prints Glk AGiliTy's software license.\n");
    }
    ...
    gagt_normal_string ("There is no help available on that Glk command."
                        "  Sorry.\n");
}
          ...
          if (!matched->takes_argument && strlen (argument) > 0)
            {
              ...
              gagt_normal_string (" command ignores arguments.]\n");
            }
        }
...
agt_input (int in_type)
{
  ...
  if (gagt_readlog_stream)
    {
      ...
      chars = glk_get_line_stream (gagt_readlog_stream, buffer, length);
      if (chars > 0)
        {
  ...
  glk_request_line_event (gagt_main_window, buffer, length - 1, 0);
  gagt_event_wait (evtype_LineInput, &event);

  /* Terminate the input line with a NUL. */
  assert (event.val1 < length);
  buffer[event.val1] = '\0';
 ...
 * event wait functions set a flag to indicate when glk_select() is active.
 */
...
gagt_get_user_file (glui32 usage, glui32 fmode, const char *fdtype)
{
  ...
  glk_request_line_event (gagt_main_window, filepath, sizeof (filepath) - 1, 0);
  gagt_event_wait (evtype_LineInput, &event);
...
gagt_get_user_file (glui32 usage, glui32 fmode, const char *fdtype)
{
   ...
   */
  if (fmode == filemode_Read && !glk_fileref_does_file_exist (fileref))
    {
      glk_fileref_destroy (fileref);
      return badfile (fSAV);
    }
...
{
  /*
   * If interpreter code is still active, and we're not in a glk_select(),
  ...
  if (gagt_agility_running && !gagt_event_in_glk_select ())
    {
      ...
      /*
       * Clear the flag to avoid recursion, and call glk_exit() to clean up
       ...
       * glk_exit().  If we have no main window, there's no point in doing
       * anything more.
       */
      if (gagt_main_window)
        {
...
gagt_exit (void)
{
   ...
   * buffered output data, and then call the real glk_exit().
   */
  gagt_agility_running = FALSE;
  gagt_output_delete ();
  glk_exit ();
...
__wrap_exit (int status)
{
   ...
   * here -- we don't, and IFP redirects glk_exit(), the only other potential
 ...
 * normal AGiliTy startup code, after which, Glk will call glk_main().
 */
static Boolean
