...
// Unix this is generally somewhere like /usr/lib/gargoyle/fonts
...
// As a last-ditch effort look "locally" for the fonts. On Unix this is
// the current working directory. On Windows, it's the directory
// containing the executable. This is primarily useful if Gargoyle is
// being distributed in a "standalone" package to run a single game,
// meaning fonts won't be installed anywhere.
static bool font_path_fallback_local(const struct font &font, char *outpath, size_t n)
{
#ifdef _WIN32
    char cwd[MAX_PATH];
    GetModuleFileName(nullptr, cwd, sizeof cwd);
    PathRemoveFileSpec(cwd);
    std::snprintf(outpath, n, "%s\\%s", cwd, font.fallback);
    return true;
#else
